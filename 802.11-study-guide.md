...
4. **802.1X / EAP** *(enterprise only)* — `EAPOL-Start` · `EAP-Request/Identity` · `…EAP method…` · `EAP-Success`\
   The AP (*authenticator*) relays EAP between the client (*supplicant*) and a RADIUS server (*authentication server*). When it succeeds, client and server both hold a **Master Session Key (MSK)**; the server hands it to the AP in the RADIUS Access-Accept. The first 256 bits become the PMK.
